...
open class EZAlertController {
    ...
    static func showAlert(title: String = "Whoops!", message: String, buttons: [String] = ["OK"], viewController: UIViewController?, tapBlock:((UIAlertAction, Int) -> Void)? = nil) {
        guard let weakViewController = viewController else {return}
        
        if message.count > 0 && (UIDevice.topMostVC as? UIAlertController)?.message != message {
            EZAlertController.alert(title, message: message, buttons: buttons) { (action, index) in
                if tapBlock != nil {
                    tapBlock!(action,index)
                }
            }.show(weakViewController, sender: nil)
        }
    }
    
    static func isInternetAvailable(showError: Bool = false, viewController: UIViewController?) -> Bool {
        let isAvailable =  Reachability.shared.isConnectedToNetwork()
        if !isAvailable && showError {
            if let weakViewController = viewController {
                EZAlertController.showAlert(message: AlertMesssage.CHECK_INTERNET_CONNECTION, viewController: weakViewController)
            }
        }
        print("Internet Avaliablity -----> \(isAvailable)")
        return isAvailable
    }
}
...
extension UIAlertAction {
    ...
    func setImage(_ image: UIImage, _ align: CATextLayerAlignmentMode? = nil) {
        setValue(image.withRenderingMode(.alwaysOriginal), forKey: "image")
        if align != nil {
            setValue(align, forKey: "titleTextAlignment")
        }
    }
}
